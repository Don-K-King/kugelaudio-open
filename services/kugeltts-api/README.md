...
- **GPU pinning**: Docker Compose uses `gpus: all` for compatibility with newer Compose validators, while `NVIDIA_VISIBLE_DEVICES` is required to restrict GPU visibility inside the container. This avoids accidental exposure of every GPU while keeping the compose file portable.
...
> Note: GPU pinning relies on `NVIDIA_VISIBLE_DEVICES` (required) in `docker-compose.yml`. Compose is configured with `gpus: all` to satisfy newer validators, so ensure the environment variable is set to avoid exposing every GPU inside the container.
