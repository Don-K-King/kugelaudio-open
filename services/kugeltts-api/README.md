...
- **GPU pinning**: Docker Compose exposes GPUs (`gpus: all`) and `NVIDIA_VISIBLE_DEVICES` limits visibility inside the container. The compose file requires `NVIDIA_VISIBLE_DEVICES` to be explicitly set (no `all` fallback) to avoid accidental exposure of every GPU.
...
> Note: GPU pinning is enforced by `NVIDIA_VISIBLE_DEVICES` inside the container while Compose exposes GPUs via `gpus: all`. The compose file still requires `NVIDIA_VISIBLE_DEVICES` to be set to avoid accidental exposure of every GPU.
